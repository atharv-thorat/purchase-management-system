...
purchase-management/
├── SPEC.md
├── README.md                     # for reviewers: problem, rules, architecture, how to run, demo script
├── DECISIONS.md                  # design decision log
├── CLAUDE.md                     # working notes for AI-assisted sessions
├── dev.sh                        # one command: backend + frontend (--reset restores the demo data)
├── docs/
│   ├── DESIGN.md                 # this file
│   ├── DEMO_CHEATSHEET.md        # one-page demo steps + likely questions
│   └── screenshots/              # regenerated by `npm run e2e:screenshots`
│
├── backend/
...
│   ├── app/
│   │   ├── main.py               # FastAPI app, CORS, router registration, error handlers, offline /docs
│   │   ├── static/swagger-ui/    # vendored Swagger UI 5 (Apache-2.0) so /docs needs no internet
│   │   ├── core/
...
│
└── frontend/
    ├── package.json              # `npm run dev` / `npm run e2e`; ./dev.sh at the root starts everything
    ├── playwright.config.ts      # E2E on its own DB and ports (:8011/:3011), isolated from the demo
    ├── e2e/                      # demo.spec.ts: full P2P flow in Chrome, role checks, offline checks
    ├── next.config.js
    ├── tailwind.config.ts
    ...
    └── src/
        ...
        │   ├── decimal.ts                # exact decimal previews (no floats; D-57)
